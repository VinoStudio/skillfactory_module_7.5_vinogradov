...
class Dot:
    def __init__(self, x: str, y: int):
        assert type(x) == int, 'Введите правильное значение!'
        assert type(y) == int, 'Введите правильное значение!'
...
class Board:
    def __init__(self, size: int, hid = False):
        assert type(size) == int, 'Введите численное значение'
        self.size = size
        self.hid = hid
...
class AI(Player):
    def ask(self):
        ...
        points = {1: 'A', 2 : 'B', 3 : 'C', 4 : 'D', 5 : 'E', 6 : 'F'}
        print(f'Ход компьютера: {points[d.x + 1]} {d.y + 1}')
        return d
    ...
    def ask(self):
        ...
        while True:
            ...
            if len(coords) != 2:
                ...
                continue
            x, y = points[coords[0].upper()], int(coords[-1])
            if not all((type(x) == int, type(y) == int)):
